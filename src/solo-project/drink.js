  ...
  return (
    <div>
      {popular.map((drink, idx) => {
        return (
            <div ><button className="cocktail-button" onClick={fetchdrinks}>Find Your Favorite</button>
          <div onClick={() => {navigate(`${drink.idDrink}`)}}>
            <div><h2>{drink.strDrink}</h2></div>
            <img style={{width:'900px', height:'900px'}} src={drink.strDrinkThumb} alt='#'/>
